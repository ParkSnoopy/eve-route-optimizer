...
impl SystemHolder {
    ...
    pub fn build_shortest_path(&self, feedback_step: usize) -> ArcRwLock<CurrentShortest> {
            ...
            Some(system_to) => {
                if system_to.read().unwrap().name() != system_from.read().unwrap().name() {
                    let system_from_index = systems
                        .iter()
                        .position(|ss| {
                            ss.read().unwrap().name() == system_to.read().unwrap().name()
                        })
                        .unwrap();
                    systems.remove(system_from_index);
                }
        ...
        let current_shortest: ArcRwLock<CurrentShortest> =
            Arc::new(RwLock::new(CurrentShortest::new()));

        systems
            .clone()
            .into_iter()
            .permutations(systems.len())
            .enumerate()
            .par_bridge()
            .for_each(|(idx, sync_route)| {
                if idx.wrapping_rem(feedback_step) == 0 {
                    crate::PROGRESS_HOLDER
                        .write()
                        .unwrap()
                        .feedback(idx as u128);
                }

                let system_from_rlock = system_from.read().unwrap();
                let mut route_length: u64 = system_from_rlock
                    .get_distance_to(&sync_route[0])
                    .expect(&trace::string::error(format!(
                        "Distance from '{}' to '{}' not set",
                ...
                match &crate::CLI_ARGS.read().unwrap().end {
                    Some(system) => {
                        route_length += sync_route[sync_route.len() - 1]
                            .read()
                            .unwrap()
                            .get_distance_to(self.get(system.name()))
                            .unwrap();
                    }
                    None => {}
                }

                sync_route.windows(2).for_each(|window| {
                ...
                current_shortest
                    .write()
                    .unwrap()
                    .register(&sync_route, route_length);
            });
